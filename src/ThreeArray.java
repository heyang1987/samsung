...
public class ThreeArray {
	...
	public void shuffle(ArrayList<Integer> userArray) throws Exception {
		...
		int userArraySize = userArray.size();
                System.out.println("User Array size: " + userArraySize);
		...
		do{
			...
			arffFunctions.generateArff(array3, "docs/samsung_header.txt", "model3.arff");
		...
		while ( (fc1.numElements() == 1) && (fc2.numElements() ==1)){
			...
			array1 = new ArrayList<>(array.subList(0, Math.round(array.size()/2)));
			array2 = new ArrayList<>(array.subList(Math.round(array.size()/2), array.size()));
			Collections.sort(array1);
			Collections.sort(array2);
		...
		while ( (fc1.numElements() == 1) && (fc3.numElements() ==1)){
			...
			Collections.shuffle(array);
			array1 = new ArrayList<>(array.subList(0, Math.round(array.size()/2)));
			array3 = new ArrayList<>(array.subList(Math.round(array.size()/2), array.size()));
			Collections.sort(array1);
			Collections.sort(array3);
...
//				System.out.println("Array3 size: " + array3.size());
		...
		while ( (fc3.numElements() == 1) && (fc2.numElements() ==1)){
			...
			array.addAll(array3);
			Collections.shuffle(array);
			array2 = new ArrayList<>(array.subList(0, Math.round(array.size()/2)));
			array3 = new ArrayList<>(array.subList(Math.round(array.size()/2), array.size()));
			Collections.sort(array2);
			Collections.sort(array3);
	...
	public void converge() throws Exception{
		...
		for (int expTimes = 0; expTimes < 200; expTimes++){
			...
			Array3Accuracy = wekaFunctions.eval(fc3, data3, data3);
			System.out.println("=============================================================================");
			...
			if ( array1.equals(array1Backup) && array2.equals(array2Backup) && array3.equals(array3Backup)){
				double accuracy = data1.numInstances()*Array1Accuracy/1359600 + data2.numInstances()*Array2Accuracy/1359600 + data3.numInstances()*Array3Accuracy/1359600;
				System.out.println("*****************************************************************************");
